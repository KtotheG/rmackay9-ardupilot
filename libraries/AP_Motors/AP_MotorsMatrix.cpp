...
void AP_MotorsMatrix::output_armed()
{
    ...
    if(_batt_voltage_max > 0 && _batt_voltage_min < _batt_voltage_max) {
        float batt_voltage = _batt_voltage + _batt_voltage * _batt_resistance;
    ...
    if (_rc_throttle.servo_out == 0) {
        ...
        if (_batt_timer < 400 && _rc_throttle.radio_out >= _hover_out) {
        ...
        yaw_allowed = max(yaw_allowed, _matrix_yaw_min);
